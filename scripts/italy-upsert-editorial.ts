/**
 * Upsert hand-curated Italy editorial guides into community_notes.
 *
 *   npm run italy:upsert-editorial
 *   npm run italy:upsert-editorial -- --dry-run
 */
...
import { ITALY_EDITORIAL_GUIDES } from "@/lib/community-notes/guides/italy-editorial-index";
...
async function main() {
  ...
  for (const guide of ITALY_EDITORIAL_GUIDES) {
    ...
    const row = {
      ...
      source_label: guide.source_label ?? "editorial:italy-seed",
      status: "published",
      published_at: now,
      updated_at: now,
    };
