 ...
 * Usage:
 *   npm run spain:daily
 *   npm run spain:daily -- --max-notes=1 --skip-drafts
 */
...
function runIncrementalParser(): CommunitySignalIngest[] {
  ...
  const result = spawnSync(python, [py, "--json-out", SIGNALS_JSON, "--country", "spain"], {
  ...
  return signals.filter((s) => s.country_key === "spain");
}
...
async function main() {
  ...
  const drafts = await publishDraftsFromNewSignals(maxNotes, "spain");
