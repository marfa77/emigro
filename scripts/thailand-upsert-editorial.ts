/**
 * Upsert the 15 hand-curated Thailand editorial guides into community_notes.
 *
 *   npm run thailand:upsert-editorial
 *   npm run thailand:upsert-editorial -- --dry-run
 */
...
import { flattenBodySections, normalizeNoteDraftSeo, validateNoteDraft } from "@/lib/community-notes/editorial-quality";
...
import { THAILAND_EDITORIAL_GUIDES } from "@/lib/community-notes/guides/thailand-editorial-index";
...
async function main() {
  ...
  for (const guide of THAILAND_EDITORIAL_GUIDES) {
    ...
    const gateErrors = validateNoteDraft({ ...draft, official_links: guide.official_links }, "thailand");
    ...
    const row = {
      ...
      source_label: guide.source_label ?? "editorial:thailand-seed",
      status: "published",
      published_at: now,
      updated_at: now,
    };
