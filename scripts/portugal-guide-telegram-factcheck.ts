/**
 * Portugal guide fact-check: query community_signals for TG practice vs pillar guides.
 *
...
} from "@/lib/guides/portugal-telegram-citations";
...
function printReport(rows: FactcheckRow[], sourceLabel: string) {
  console.log(`\n=== Portugal guide fact-check (${sourceLabel}) ===`);
  console.log(`Signals: ${rows.length}\n`);

  if (rows.length === 0) {
    console.log("No matching signals. Try --seed-only or run npm run portugal:daily first.");
    return;
  }
...
async function main() {
  ...
  if (configs.length === 0) {
    ...
    console.error("Available aliases:", PORTUGAL_GUIDE_FACTCHECK.flatMap((g) => [g.slug, ...g.aliases]).join(", "));
    process.exit(1);
  }
  ...
  console.log("\nNext: embed citations with lib/guides/portugal-telegram-citations.ts → formatPortugalChatCitation()");
  console.log("Docs: docs/PORTUGAL_GUIDE_FACTCHECK.md");
}
