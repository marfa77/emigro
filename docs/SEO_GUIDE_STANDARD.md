...
- Видимый **короткий ответ** (`quick_answer`) в начале; факты для LLM — `data-llm` **sr-only**. Не дублировать chrome «Коротко для проверки маршрута» (см. `.cursor/rules/seo-geo-aeo-checklist.mdc`)
- Ссылка на `/llms.txt`
...
- [ ] FAQ совпадает со schema FAQPage (только если на странице ≥5 видимых вопросов)
- [ ] CTA на hub wizard
- [ ] Короткий ответ в начале; LLM-факты sr-only — без второго видимого recap-chrome
