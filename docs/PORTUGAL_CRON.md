...
**OG/hero images** (07:30 UTC): `/api/cron/portugal-note-images` — прогревает Pexels WebP для новых гайдов, опубликованных VPS-ом без файла в `public/`. На Vercel (read-only FS) картинка отдаётся через `/api/community-notes/hero/{slug}` с CDN-кэшем. Для статического CDN-пути: VPS/local генерирует WebP → slug попадает в `note-og-slugs.ts` → `git commit` + deploy.

На Vercel нужен `PEXELS_API_KEY` (и `CRON_SECRET` для cron).
