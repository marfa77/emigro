#!/usr/bin/env bash
# Daily Italy community pipeline (VPS systemd — same pattern as Spain).
#
# Prod: systemd timer emigro-italy-community.timer (08:00 UTC).
#
...
LOCK_FILE="$PARSER_ROOT/.scheduled-italy.lock"
...
exec >> "$LOG_DIR/scheduled-italy-$(date +%Y%m%d).log" 2>&1
...
export ITALY_SATELLITE_USE_SUBDOMAIN="${ITALY_SATELLITE_USE_SUBDOMAIN:-true}"
export EMIGRO_PUBLIC_SITE_URL="${EMIGRO_PUBLIC_SITE_URL:-https://www.emigro.online}"

log "=== Emigro Italy daily (age<=${PARSER_MAX_AGE_HOURS}h, limit=${PARSER_INCREMENTAL_LIMIT}/ch) ==="
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    log "=== Italy daily skipped (already running) ==="
    exit 0
  fi
fi
...
if [[ "$code" -eq 0 ]]; then
  log "=== Italy daily finished OK ==="
  exit 0
fi

log "=== Italy daily FAILED (exit $code) ==="
exit "$code"
