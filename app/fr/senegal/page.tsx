...
export const metadata: Metadata = pageMetadata({
  title: "Résidence en France pour Sénégalais 2026",
  description:
    "Comment émigrer en France depuis le Sénégal 2026 : visa ≠ résidence, VLS-TS, Passeport Talent et naturalisation ~5 ans. Sources officielles.",
  path: FR_PATHS.senegal,
  locale: "fr",
  frHreflang: { originIso: "SN", destinationIso: "FR" },
  aiDescription:
    "Sénégal→France hub: short-stay visa vs national long-stay (VLS-TS), Talent passport, consulates, naturalisation ~5 years.",
  aiCategory: "origin-corridor-hub",
});

export default function FrSenegalHubPage() {
  const guides = listGuides("fr").filter((g) =>
    (FR_SN_GUIDE_SLUGS as readonly string[]).includes(g.slug),
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-corridor-600">
          Corridor {FR_SN_FRANCE_CORRIDOR.slug}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">Sénégal → France</h1>
        <p className="mt-4 text-lg text-slate-700">
          Afrique de l&apos;Ouest francophone → France. Visa long séjour / Talent. Naturalisation ~5 ans — pas un
          raccourci « 2 ans ».
        </p>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
          <p>
            Passeports : <strong>{FR_SN_FRANCE_CORRIDOR.passports.join(", ")}</strong>. Aussi :{" "}
