...
export const metadata: Metadata = pageMetadata({
  title: "Résidence en France pour Algériens 2026",
  description:
    "Comment émigrer en France depuis l'Algérie 2026 : visa ≠ résidence, VLS-TS, Passeport Talent et naturalisation ~5 ans. Sources officielles.",
  path: FR_PATHS.algerie,
  locale: "fr",
  frHreflang: { originIso: "DZ", destinationIso: "FR" },
  aiDescription:
    "Algérie→France hub: short-stay visa vs national long-stay (VLS-TS), Talent passport, apostille since July 2026, naturalisation ~5 years.",
  aiCategory: "origin-corridor-hub",
});

export default function FrAlgerieHubPage() {
  const guides = listGuides("fr").filter((g) =>
    (FR_DZ_GUIDE_SLUGS as readonly string[]).includes(g.slug),
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-corridor-600">
          Corridor {FR_DZ_FRANCE_CORRIDOR.slug}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">Algérie → France</h1>
        <p className="mt-4 text-lg text-slate-700">
          Visa + titre de séjour. Vérifiez le régime applicable et l&apos;apostille (HCCH : entrée en vigueur pour
          l&apos;Algérie le 9 juillet 2026). Naturalisation ~5 ans — pas un « 2 ans Maghreb ».
        </p>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
          <p>
            Passeports : <strong>{FR_DZ_FRANCE_CORRIDOR.passports.join(", ")}</strong>. Aussi :{" "}
            <Link href={FR_PATHS.maroc} className="text-corridor-700 hover:underline">
              Maroc → France
            </Link>
            {" · "}
            <Link href={FR_PATHS.tunisie} className="text-corridor-700 hover:underline">
              Tunisie → France
            </Link>
            {" · "}
