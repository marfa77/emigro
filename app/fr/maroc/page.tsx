...
export const metadata: Metadata = pageMetadata({
  title: "Résidence en France pour Marocains 2026",
  description:
    "Comment émigrer en France depuis le Maroc 2026 : visa ≠ résidence, VLS-TS, Passeport Talent et naturalisation ~5 ans. Sources officielles.",
  path: FR_PATHS.maroc,
  locale: "fr",
  frHreflang: { originIso: "MA", destinationIso: "FR" },
  aiDescription:
    "Maroc→France hub: short-stay visa vs national long-stay (VLS-TS), Talent passport, consulates, naturalisation ~5 years.",
  aiCategory: "origin-corridor-hub",
});

export default function FrMarocHubPage() {
  const guides = listGuides("fr").filter((g) =>
    (FR_MA_GUIDE_SLUGS as readonly string[]).includes(g.slug),
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-corridor-600">
          Corridor {FR_MA_FRANCE_CORRIDOR.slug}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">Maroc → France</h1>
        <p className="mt-4 text-lg text-slate-700">
          Visa Schengen généralement requis. Résider exige un VLS-TS (consulat) : Passeport Talent, salarié,
          étudiant, visiteur… Naturalisation ~5 ans — pas un raccourci « 2 ans Maghreb ».
        </p>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
          <p>
            Passeports : <strong>{FR_MA_FRANCE_CORRIDOR.passports.join(", ")}</strong>. Aussi :{" "}
