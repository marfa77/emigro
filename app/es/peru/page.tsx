...
export const metadata: Metadata = pageMetadata({
  title: "Perú → España: residencia y rutas 2026",
  description:
    "Hub para peruanos que quieren vivir en España: estancia corta vs residencia, nómada digital, no lucrativa y checklist de llegada.",
  path: ES_PATHS.peru,
  locale: "es",
  esHreflang: { originIso: "PE", destinationIso: "ES" },
});

export default function EsPeruHubPage() {
  const guides = listGuides("es").filter((g) =>
    (ES_PE_GUIDE_SLUGS as readonly string[]).includes(g.slug),
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-corridor-600">
          Corredor {ES_PE_SPAIN_CORRIDOR.slug}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">
          Perú → España
        </h1>
        <p className="mt-4 text-lg text-slate-700">
          Pasaporte peruano: suele poder visitar Schengen en corta estancia, pero{" "}
          <strong>vivir y trabajar</strong> exige una vía de residencia. Demanda real con SEO menos
          saturado que MX/CO/AR/VE.
        </p>
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 flex flex-wrap gap-4 text-sm font-medium">
            ...
            <Link href={ES_PATHS.ecuador} className="inline-flex items-center gap-2 text-corridor-700 hover:underline">
              Comparar con Ecuador
              <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </section>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
          <p>
            Pasaportes: <strong>{ES_PE_SPAIN_CORRIDOR.passports.join(", ")}</strong>. También:{" "}
