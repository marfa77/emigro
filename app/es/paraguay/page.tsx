...
export const metadata: Metadata = pageMetadata({
  ...
  description:
    "Cómo emigrar a España desde Paraguay 2026: Schengen corto ≠ residencia. Nómada digital (~€2.849), no lucrativa y nacionalidad en 2 años (art. 22).",
  path: ES_PATHS.paraguay,
  locale: "es",
  esHreflang: { originIso: "PY", destinationIso: "ES" },
  aiDescription:
    "Paraguay→Spain hub: short Schengen stay vs national residence (digital nomad, non-lucrative), Spanish nationality ~2 years (art. 22). Clean niche Cono Sur wedge.",
  aiCategory: "origin-corridor-hub",
});

export default function EsParaguayHubPage() {
  const guides = listGuides("es").filter((g) =>
    (ES_PY_GUIDE_SLUGS as readonly string[]).includes(g.slug),
  );
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-corridor-600">
          Corredor {ES_PY_SPAIN_CORRIDOR.slug}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">
          Paraguay → España
        </h1>
        <p className="mt-4 text-lg text-slate-700">
          Pasaporte paraguayo: wedge limpio (como Uruguay). Visitar Schengen ≠{" "}
          <strong>vivir y trabajar</strong> — hace falta una vía de residencia.
        </p>

        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-950">Qué cubrimos</h2>
          <ul className="mt-4 space-y-2 text-slate-700">
            <li>• Estancia corta vs residencia</li>
            <li>• Rutas 2026 para pasaporte PY</li>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
          <p>
            Pasaportes: <strong>{ES_PY_SPAIN_CORRIDOR.passports.join(", ")}</strong>. También:{" "}
