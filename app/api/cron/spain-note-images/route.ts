...
/** Warm OG/hero images for Spain guides (PEXELS on Vercel, dynamic /api route). */
export async function GET(request: Request) {
  ...
  const lookbackDays = Math.min(parseInt(process.env.SPAIN_NOTE_IMAGE_LOOKBACK_DAYS ?? "30", 10) || 30, 60);
  ...
  try {
    ...
    for (const note of recentGuides.slice(0, 7)) {
      try {
    ...
    await trackServerEvent("cron_spain_note_images_error", { message }, "cron");
