...
echo "📘 Deploy Emigro Threads daily + refresh + replies → ${SERVER_USER}@${SERVER_HOST}:${REMOTE}"
...
set -euo pipefail
cd "${REMOTE}"
chmod +x deploy/threads-daily/run_scheduled.sh deploy/threads-refresh/run_scheduled.sh deploy/threads-replies/run_scheduled.sh
mkdir -p deploy/threads-daily/logs deploy/threads-refresh/logs deploy/threads-replies/logs parser/out
chown -R www-data:www-data deploy/threads-daily deploy/threads-refresh deploy/threads-replies parser/out
...
echo "✅ Threads timers enabled (daily + refresh + replies DM)."
