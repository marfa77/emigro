...
function OpenLink({ href, external, comingSoon }: { href: string; external?: boolean; comingSoon?: boolean }) {
  const className =
    "inline-flex items-center gap-0.5 rounded-full bg-white/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-white backdrop-blur-sm";
        ...
        <div className="absolute inset-0 overflow-hidden rounded-2xl [backface-visibility:hidden] [transform:rotateY(180deg)]">
          ...
          <div className="relative flex h-full flex-col p-4 text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
            ...
            <div className="mt-auto flex items-center justify-between pt-3">
              <p className="text-[10px] uppercase tracking-wider text-white/50">Emigro · {tile.hubLabel}</p>
              <OpenLink href={tile.href} external={tile.external} comingSoon={tile.comingSoon} />
            </div>
          </div>
        </div>
...
export function CorridorHubTilesLegend() {
  return (
    <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-slate-400">
      ...
      Клик — рейтинги слоя · Open — перейти в раздел · Soon — скоро
    </p>
  );
}
