...
export async function CorridorHubPulse({ topic }: Props) {
  const pulse = await getCorridorWizardPulse(topic.urlSegment, topic.corridorSlug);
  ...
  return (
    <section
      ...
      aria-labelledby={`corridor-hub-pulse-${topic.urlSegment}`}
    >
      <PulseHeader
        ...
        headingId={`corridor-hub-pulse-${topic.urlSegment}`}
      />
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-black/20 p-4">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h4 className="text-sm font-semibold uppercase tracking-wide text-teal-200">
              ...
              {pulse.countryLabel} · программы
            </h4>
            <p className="text-xs text-slate-500">{pulse.corridorChecks} проверок по маршруту</p>
          </div>
          <ul className="mt-3 space-y-2">
            {pulse.topPrograms.map((item) => (
