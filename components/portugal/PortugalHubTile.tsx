...
import type { HubTileIcon, HubTileRating, ResolvedHubTile } from "@/lib/portugal/hub";
...
export function PortugalHubTile({ tile }: Props) {
  ...
  return (
    <div
      className={`group relative aspect-[4/5] w-full min-h-[220px] [perspective:1000px] sm:aspect-[3/4] sm:min-h-[260px] ${
      ...
      aria-label={
        score
          ? `${tile.title}, Emigro Score ${score.overall100} — нажмите для осей`
          : `${tile.title}${canFlip ? " — нажмите для деталей" : ""}`
      }
      onClick={() => {
        ...
        <div className="absolute inset-0 overflow-hidden rounded-2xl [backface-visibility:hidden]">
          ...
          <div className="relative flex h-full flex-col p-4 text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.45)]">
            <div className="flex items-start justify-between gap-2">
              <div>
                {score ? (
                  <>
                    ...
                    <p className="mt-0.5 text-[10px] uppercase tracking-[0.18em] text-white/55">Emigro Score</p>
                  </>
                ) : (
                  <>
              ...
              <div className="flex shrink-0 items-center gap-1 rounded-full bg-black/25 px-2 py-1 text-[11px] font-medium backdrop-blur-sm">
...
export function PortugalHubTilesLegend() {
  return (
    <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-slate-400">
      ...
      <Link href="/ru/emigro-score" className="text-corridor-600 hover:underline">
        методология
      </Link>
    </p>
  );
